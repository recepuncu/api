...
function product_add() {
    ...
    try {
		...
		$product = $db->get("product", "*", array("OR" => array("model" => $request->data["model"], "sku" => $request->data["sku"])));
        if ($product) {
			...
			foreach ($request->data["category_ids"] as $category_id) {
				$db->update("product_to_category", array(
...
function update_price() {
    ...
    try {
        foreach ($request_data as $data) {
            $updated = $db->update("product", array("price" => $data["lFiyat"]), array("model" => $data["sModel"]));
        }
        Flight::json(array('state' => true, 'data' => sprintf('%s tane fiyat güncellendi.', $updated), 'Messages' => array()));
    } catch (Exception $e) {
        Flight::json(array('state' => false, 'data' => array(), 'Messages' => array('db' => $db->error(), 'ex' => $e->getMessage())));
    }
}
